...
const FALLBACK_TEXT = 'the quick brown fox jumps over the lazy dog please type this sentence as fast as you can go';
...
const generateGameText = async (): Promise<string> => {
  ...
  if (!data || data.length === 0) return FALLBACK_TEXT;

  let words: string[] = [];
  for (const row of data) {
    if (Array.isArray(row.word)) {
      words = words.concat(row.word);
    } else if (typeof row.word === 'string' && row.word.trim()) {
      words.push(row.word.trim());
    }
  }

  if (words.length === 0) return FALLBACK_TEXT;
